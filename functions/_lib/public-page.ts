...
const ASSET_REVISION = '20260523-stability-3';
...
function readerThemeScript() {
  return `<script>(()=>{try{const p=localStorage.getItem('rrp-reader-theme')||'dark';const m=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches;const t=p==='system'?(m?'dark':'light'):p;document.documentElement.dataset.theme=t==='light'?'light':'dark';document.documentElement.dataset.readerLineHeight=localStorage.getItem('rrp-reader-line-height')||'normal';document.documentElement.dataset.readerContentWidth=localStorage.getItem('rrp-reader-content-width')||'comfortable'}catch{document.documentElement.dataset.theme='dark'}document.addEventListener('click',(e)=>{const b=e.target.closest('[data-theme-toggle]');if(!b)return;const n=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=n;localStorage.setItem('rrp-reader-theme',n)})})()</script>`;
}
