...
import { adminGuard, getWorkRelations, normalizeWorkPayload, notFound, syncTaxonomy, workByIdOrSlug } from '../../../_lib/editorial';
...
async function updateWork({ request, env, params }: any) {
  try {
    ...
    await db.prepare(`
      UPDATE works SET
        slug = ?, title = ?, subtitle = ?, short_description = ?, description = ?, seo_title = ?, seo_description = ?,
      ...
      WHERE id = ?
    `).bind(
      data.slug, data.title, data.subtitle, data.short_description, data.description, data.seo_title, data.seo_description,
...
export async function onRequestDelete({ request, env, params }: any) {
  try {
    ...
    const work = await workByIdOrSlug(db, params.id);
    if (!work) return notFound('Obra não encontrada.');
    await db.prepare('DELETE FROM works WHERE id = ?').bind(work.id).run();
    return json({ ok: true, message: 'Obra excluída com sucesso.' });
  } catch (error) {
    return json({ ok: false, message: error instanceof Error ? error.message : 'Erro ao excluir obra.' }, { status: 500 });
  }
}
