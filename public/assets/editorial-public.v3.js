(() => {
  ...
  function initCarousel(banners) {
    ...
    const MIN_AUTOPLAY_INTERVAL = 5000;
    const MAX_AUTOPLAY_INTERVAL = 20000;
    ...
    window.__RRP_HOME_CAROUSEL_CLEANUP__ = cleanup;
    ...
    root.innerHTML = `
      <div class="carousel-track" data-carousel-track>
        ${banners.map((banner, index) => `<article class="carousel-slide ${index === 0 ? 'is-active' : ''} ${banner.uses_cover_only ? 'uses-cover-fallback' : ''}" data-carousel-slide aria-hidden="${index !== 0}">
          <picture>
    ...
    const delayForActiveSlide = () => {
      const requested = Number(banners[active]?.duration_ms);
      return Number.isFinite(requested) && requested > 0
        ? Math.max(MIN_AUTOPLAY_INTERVAL, Math.min(MAX_AUTOPLAY_INTERVAL, requested))
    ...
    const mayAutoplay = () => !reduceMotion && !document.hidden && !root.matches(':hover') && !root.contains(document.activeElement);
    ...
    $('[data-carousel-prev]', root)?.addEventListener('click', () => interact(active - 1), { signal });
    $('[data-carousel-next]', root)?.addEventListener('click', () => interact(active + 1), { signal });
    dots.forEach((dot) => dot.addEventListener('click', () => interact(Number(dot.dataset.carouselDot)), { signal }));
    root.addEventListener('mouseenter', clearAutoplay, { signal });
    root.addEventListener('mouseleave', scheduleAutoplay, { signal });
    root.addEventListener('focusin', clearAutoplay, { signal });
    root.addEventListener('focusout', (event) => {
      if (!root.contains(event.relatedTarget)) scheduleAutoplay();
    }, { signal });
    root.addEventListener('keydown', (event) => {
      if (event.key === 'ArrowLeft') { event.preventDefault(); interact(active - 1); }
      if (event.key === 'ArrowRight') { event.preventDefault(); interact(active + 1); }
    }, { signal });
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) clearAutoplay(); else scheduleAutoplay();
    }, { signal });
